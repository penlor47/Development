...
struct StoryBrain {
    ...
    mutating func nextStory(_ userAnswer: String) {
        if userAnswer == story[storyBranch].choice1 {
            storyBranch = story[storyBranch].choice1Destination
        } else if userAnswer == story[storyBranch].choice2 {
            storyBranch = story[storyBranch].choice2Destination
        }
    }
    
    func storyText() -> String {
        return story[storyBranch].title
    }
    func choice1Text() -> String {
        
        return story[storyBranch].choice1
    }
    func choice2Text() -> String {
        return story[storyBranch].choice2
    }
